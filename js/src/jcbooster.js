export default function JcApp() {
	...
	jc.event = function(evname, cb) {
		document.addEventListener(evname, res => {
			cb(res);
		});
	};
	jc.regEvent = document.createEvent("Event");
	...
	jc.init = function(Doc) {
			...
			if (expandAct !== null) {
				expandAct.forEach(function(el) {
					...
					if (roll !== null) {
						if (roll == groupRoll) {
							let heightNode = 0;
							el.style.cssText = `max-height: ${heightNode.toString()}px;`;
			...
			if (target !== null) {
				el.addEventListener('click', function() {
					...
					if (exAct == false) {
						...
						tg.setAttribute('style', `max-height: ${heightNode.toString()}px;`);
						setTimeout( function() {
						...
						heightNode = 0;
						tg.style.cssText = `max-height: ${heightNode.toString()}px;`;
		...
		let setAnimationTabs = function(el, anim) {
			anim = ( (anim == undefined) ? "fadeIn" : anim);

			let cls = anim;
			let anm = "animated";
			let spd = "faster";
			( !el.classList.contains(anm) && el.classList.add(anm) );
			( !el.classList.contains(spd) && el.classList.add(spd) );
			( el.classList.contains(cls) && el.classList.remove(cls) );
			( !el.classList.contains(cls) && el.classList.add(cls) );
		};
		Doc.querySelectorAll('.tabs').forEach( btn => {
			btn.onclick = e => {
				let cls = "tabs-active";
				let roll = btn.getAttribute('tabs-roll');
				queryAll('.tabs-group').forEach(el => {
					if ( el.getAttribute('roll') == roll ) {
						( el.classList.contains(cls) && el.classList.remove(cls) );
					}
				});
				let frmElem = query( btn.getAttribute('tabs-target') );
				setAnimationTabs(frmElem);
				frmElem.classList.toggle(cls);
			}
		});
		...
		Doc.querySelectorAll('.modal-toggle').forEach(function(el) {
			el.addEventListener('click', function() {
				...
				if ( modal.classList.contains("modal-show") ) {
					jc.regEvent.content = modal;
					jc.regEvent.initEvent("modalShow", true, true);
					document.dispatchEvent(jc.regEvent);
				}
				else {
					jc.regEvent.content = modal;
					jc.regEvent.initEvent("modalHide", true, true);
					document.dispatchEvent(jc.regEvent);
				}
				if ( iW < 767.98 ) {
	...
	jc.setForm = function(elem, cf) {
		let sorot = false;
		let decode = false;
		if ( cf != undefined ) {
			sorot = (cf.mark)?cf.mark:false;
			decode = (cf.decode)?cf.decode:false;
		};
			...
			elem.querySelectorAll('*').forEach( (el, i) => {
				if ( el.getAttribute('name') != null ) {
					let index = el.getAttribute('name'),
						val = ((decode)?escape(el.value):el.value);
					if ( index.replace(/ /g, '') == "" ) {
					...
					if ( el.getAttribute('required') != null) {
						...
						if ( val.length == 0 ) {
							...
							noteMark += index + ", "; 
							dataForm.status = false;
							dataForm.message = "Input Kosong! [" + noteMark + "]";
							dataForm.element = el;
							if ( sorot != false ) {
